...
class YouTubeSearchTool:
    ...
    With 3 keys from 3 separate Google Cloud projects, the effective daily limit
    is 30,000 units — more than sufficient for a single agent run.
    """
...
class DuplicateTracker:
    """
    Code-level dedup across search results.
    Marks duplicate entries with '_duplicate': True so the LLM can make
    informed decisions without silently losing data.
    """

    def __init__(self):
        self._seen_youtube: set[str] = set()
        self._seen_bilibili: set[str] = set()
        self._seen_urls: set[str] = set()

    def _get_seen_set(self, source: str) -> set[str]:
        if source == "youtube":
            return self._seen_youtube
        if source == "bilibili":
            return self._seen_bilibili
        return self._seen_urls

    @staticmethod
    def _extract_key(result: dict, source: str) -> str | None:
        if source == "youtube":
            url = result.get("url", "")
            if "v=" in url:
            ...
            return url or None
        if source == "bilibili":
            return result.get("bvid") or None
        return result.get("url") or None

    def filter_results(self, results: list[dict], source: str) -> list[dict]:
        filtered = []
        seen = self._get_seen_set(source)
        for r in results:
            key = self._extract_key(r, source)
...
class FileWriter:
    ...
    def write(self, relative_path: str, content: str) -> str:
        ...
        with open(full_path, "w", encoding="utf-8") as f:
            f.write(content)
        logger.info(f"Written: {full_path} ({len(content)} chars)")
