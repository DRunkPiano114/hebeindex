...
收集完成后，可以对现有数据按视频实际内容重新分类（而非按搜索词归属）。脚本会自动合并去重、用规则匹配 ~82% 的视频，剩余由 AI 判断。

```bash
uv run python reclassify.py --apply
```

分类结果直接写回 `processed/` 目录，前端重新构建即可看到更新。

> 需要 `OPENROUTER_API_KEY`，约 45 次 AI 调用，费用极低。加 `--no-llm` 可跳过 AI，仅用规则分类。
